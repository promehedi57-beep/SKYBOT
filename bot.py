...
OTP_GROUP_LINK = "https://t.me/SMSSKYOTP"
...
def main_menu(user_id: int):
    ...
    builder.row(types.KeyboardButton(text="📊 Live Service Range"))
...
# ================= ADMIN PANEL =================
@dp.message(F.text == "𝑨𝑫𝑴𝑰𝑵 𝑷𝑨𝑵𝑬𝑳")
async def admin_panel_button(message: types.Message):
    if not is_admin(message.from_user.id):
        await message.answer("❌ You are not an admin.")
        return
    await message.answer("𝑨𝑫𝑴𝑰𝑵 𝑷𝑨𝑵𝑬𝑳", reply_markup=admin_menu(), parse_mode="Markdown")
...
async def sync_ranges_cb(callback: types.CallbackQuery):
    ...
    await asyncio.sleep(2)
    await callback.message.edit_text("𝑨𝑫𝑴𝑰𝑵 𝑷𝑨𝑵𝑬𝑳", reply_markup=admin_menu(), parse_mode="Markdown")
